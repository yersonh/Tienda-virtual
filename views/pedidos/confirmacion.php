...
<main class="container py-5">
    <div class="mx-auto" style="max-width: 720px;">
        <div class="p-4 border rounded" style="background: var(--card-bg); border-color: var(--border) !important;">
            <h1 class="mb-3" style="font-family: 'Syne', sans-serif;"><?= htmlspecialchars(t('order_confirmed'), ENT_QUOTES, 'UTF-8') ?></h1>
            ...
            <div class="d-flex justify-content-between mb-4">
                <span class="text-secondary"><?= htmlspecialchars(t('subtotal'), ENT_QUOTES, 'UTF-8') ?></span>
                <strong>$<?= number_format((float) ($pedido['subtotal'] ?? $pedido['total'] ?? 0)) ?> COP</strong>
            </div>
            <?php if (isset($pedido['iva'])): ?>
                <div class="d-flex justify-content-between mb-2">
                    <span class="text-secondary"><?= htmlspecialchars(t('vat'), ENT_QUOTES, 'UTF-8') ?> 19%</span>
                    <strong>$<?= number_format((float) $pedido['iva']) ?> COP</strong>
                </div>
            <?php endif; ?>
            <?php if (isset($pedido['envio'])): ?>
                <div class="d-flex justify-content-between mb-2">
                    <span class="text-secondary"><?= htmlspecialchars(t('shipping'), ENT_QUOTES, 'UTF-8') ?></span>
                    <strong>$<?= number_format((float) $pedido['envio']) ?> COP</strong>
                </div>
            <?php endif; ?>
            <div class="d-flex justify-content-between mb-4">
                <span class="text-secondary"><?= htmlspecialchars(t('total'), ENT_QUOTES, 'UTF-8') ?></span>
                <strong>$<?= number_format((float) $pedido['total']) ?> COP</strong>
            </div>
            <?php renderEntregaBox($pedido['fecha_estimada_entrega'] ?? null); ?>

            <a class="btn btn-success" href="index.php?action=tienda"><?= htmlspecialchars(t('back_to_store'), ENT_QUOTES, 'UTF-8') ?></a>
        </div>
    </div>
</main>
