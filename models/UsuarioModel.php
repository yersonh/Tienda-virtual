...
class UsuarioModel
{
    ...
    // DDL requerido (ejecutar una sola vez en Oracle):
    // CREATE TABLE cuenta_reactivaciones (
    //     id          NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    //     usuario_id  NUMBER NOT NULL,
    //     token       VARCHAR2(64) NOT NULL,
    //     expires_at  TIMESTAMP NOT NULL,
    //     used        NUMBER(1) DEFAULT 0 NOT NULL,
    //     CONSTRAINT uq_reac_token UNIQUE (token),
    //     CONSTRAINT fk_reac_usuario FOREIGN KEY (usuario_id) REFERENCES usuario(id_usuario)
    // );

    public function buscarPorIdentificador(string $valor): ?array
    {
        $normalizado = strtolower(trim($valor));
        $telefono    = preg_replace('/\D/', '', $valor);

        $query = "SELECT u.id_usuario, u.username, p.nombres, p.correo,
                         UPPER(TRIM(u.estado)) AS estado
                  FROM usuario u
                  JOIN persona p ON u.id_persona = p.id_persona
                  WHERE (
                      LOWER(TRIM(u.username)) = :usr
                      OR LOWER(TRIM(p.correo)) = :eml
                      OR REGEXP_REPLACE(p.telefono, '[^0-9]', '') = :tel
                  )
                  FETCH FIRST 1 ROWS ONLY";

        $stmt = oci_parse($this->conn, $query);
        oci_bind_by_name($stmt, ':usr', $normalizado);
        oci_bind_by_name($stmt, ':eml', $normalizado);
    ...
    public function generarTokenReactivacion(int $usuarioId): string
    {
        $token = bin2hex(random_bytes(32));

        $sql = "INSERT INTO cuenta_reactivaciones (usuario_id, token, expires_at)
                VALUES (:usuario_id, :token, CURRENT_TIMESTAMP + INTERVAL '1' DAY)";

        $stmt = oci_parse($this->conn, $sql);
        oci_bind_by_name($stmt, ':usuario_id', $usuarioId, -1, SQLT_INT);
        oci_bind_by_name($stmt, ':token', $token);
        oci_execute($stmt);
        oci_free_statement($stmt);
    ...
    public function validarTokenReactivacion(string $token): ?array
    {
        $sql = "SELECT cr.usuario_id, u.username, p.nombres, p.correo
                FROM cuenta_reactivaciones cr
                JOIN usuario u ON cr.usuario_id = u.id_usuario
                JOIN persona p ON u.id_persona = p.id_persona
    ...
    public function marcarTokenReactivacionUsado(string $token): void
    {
        $sql = "UPDATE cuenta_reactivaciones SET used = 1 WHERE token = :token";
        $stmt = oci_parse($this->conn, $sql);
        oci_bind_by_name($stmt, ':token', $token);
        oci_execute($stmt);
        oci_free_statement($stmt);
    ...
    {
        $sql = "UPDATE usuario SET estado = 'Activo', updated_at = SYSDATE WHERE id_usuario = :id_usuario";
        $stmt = oci_parse($this->conn, $sql);
        oci_bind_by_name($stmt, ':id_usuario', $idUsuario, -1, SQLT_INT);
        ...
        if (!@oci_execute($stmt, OCI_NO_AUTO_COMMIT)) {
            $error = oci_error($stmt);
            oci_free_statement($stmt);
            throw new \Exception($error['message'] ?? 'No se pudo reactivar la cuenta');
        }
